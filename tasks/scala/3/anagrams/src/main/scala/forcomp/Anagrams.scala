...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    //println(w.groupBy((c:Char)=>c.toLower))
    w.groupBy((c: Char) => c.toLower).
      map(mapper => (mapper._1, mapper._2.length)).
      toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val str = occurrencesToString(occurrences)
    strCombinations(str).toSet.map(wordOccurrences).toList
  }

  def strCombinations(str: String): List[String] = {
    @tailrec
    def combinations(str: String, list: List[String]): List[String] = {
      if (str.isEmpty) list
      else {
        val newList = list.map(_ + str.head) // 加下一个字符
        combinations(str.tail, newList ::: list) // 不加下一个字符
      }
    }
  ...
  def occurrencesToString(occurrences: Occurrences): String = {
    occurrences.foldLeft("")((string, map) => string + map._1.toString * map._2)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if (y.isEmpty) x
      ...
      y.foldLeft(x.toMap) {
        case (map, (char, count)) =>
          //println("Map: " + map + "\t(char, count): " + char + "  " + count)
