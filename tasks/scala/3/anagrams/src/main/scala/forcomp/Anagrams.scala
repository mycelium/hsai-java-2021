...
object Anagrams {
   ...
   */
  def unite(oc1: Occurrences, oc2: Occurrences): Occurrences = {
    // unite 2 lists with summing frequencies
    oc1 match {
      case Nil => oc2
      case (c1, i1) :: rest1 => oc2 match {
        case Nil => oc1
        case (c2, i2) :: rest2 => if (c1 == c2) (c2, i1 + i2) :: unite(rest1, rest2)
        else if (c1 < c2) (c1, i1) :: unite(rest1, oc2)
        else (c2, i2) :: unite(oc1, rest2)
      }
    }
  }
  ...
  def wordOccurrences(w: Word): Occurrences = {
    @tailrec
    def rec(word: Word, occurrences: Occurrences): Occurrences =
      if (word.isEmpty) occurrences
      else rec(word.substring(1),
        unite(List((word.charAt(0).toLower, 1)), occurrences))
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def rec(sent: Sentence, oc: Occurrences) : Occurrences = {
      sent match {
        case Nil => oc
        case word :: rest => unite(wordOccurrences(word), rec(rest, oc))
      }
    }
    rec(s, Nil)
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    @tailrec
    def makeMap(old: Map[Occurrences, List[Word]], words: List[Word]): Map[Occurrences, List[Word]] = {
      words match {
        case Nil => old
        case w :: rest =>
